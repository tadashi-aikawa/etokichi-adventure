...
5. スマートフォン相当の横画面で、「ゲームスタート」のタップからフルスクリーンへ入り、左右の操作レールと両者の4技・選択技名・ガッツ・命中率が収まり、十字キーの中央・各方向・斜め境界で誤入力がなく、長押しと解除、技切替、技、PUSHが反応する
6. Fullscreen APIが使えない、または要求が拒否された環境でも、通常表示のままキャラクター選択へ進める
7. スマートフォン相当の横画面でアステールの魔眼を石化まで発動し、ビームと短いフラッシュの後はバトル画面の色が戻り、石化中の相手だけが白黒化し、10秒後に通常色へ戻る
